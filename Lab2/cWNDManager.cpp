...
bool cWNDManager::createWND(int width, int height, int bpp)
{
	...
	m_hwnd = CreateWindowEx(NULL,                                 // extended style
		"winOGL",                          // class name
		"Cube Using VBO OpenGL Window",      // app name
...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		// Lab 2
		if (wParam == 'd' || wParam == 'D') // If d/D is pressed
		{
			drawMode = ++drawMode % 3;
		...
		{
			rotationAngle = rotationAngle + 2; // Increase rotation angle
			glRotatef(rotationAngle, 0.0f, 1.0f, 0.0f);
		}
		if (wParam == VK_RIGHT) // If the Right Arrow was pressed
		{
			rotationAngle = rotationAngle - 2; // Decrease rotation angle
			glRotatef(-rotationAngle, 0.0f, 1.0f, 0.0f);
		}
		...
		if (wParam == VK_DOWN) // If the DOWN Arrow was pressed
		{
			rotationAngle = rotationAngle - 2; // Decrease rotation angle
			glRotatef(-rotationAngle, 1.0f, 0.0f, 0.0f);
		}
